/* This file is taken from https://github.com/iliailia/comparison-circuit-over-fq */
...
void Comparator::compute_poly_params()
{
	...
	// to choose this number as something other than a power of two and still 
	// maintain optimal depth, in principle we can try all possible values of 
	// m_babystep_num between two consecutive powers of two and choose the one 
...
{
	cout << "Creating comparison polynomial ..." << endl << endl;
	// get p
...
void Comparator::extract_mod_p(vector<Ctxt>& mod_p_coefs, const Ctxt& ctxt_x) const
{
	...
	if (m_slotDeg == 1)
	{
		// cout << "the field extension degree is " << m_slotDeg << endl;
...
Comparator::Comparator(const Context& context, 
					   CircuitType type, 
					   unsigned long d, 
					   unsigned long expansion_len, 
					   const SecKey& sk, 
					   bool verbose): m_context(context), m_type(type), m_slotDeg(d), m_expansionLen(expansion_len), m_sk(sk), m_pk(sk), m_verbose(verbose)
{
...
void Comparator::evaluate_univar_less_poly(Ctxt& ret, Ctxt& ctxt_p_1, const Ctxt& x) const
{
	...
	if (p > ZZ(3)) //if p > 3, use the generic Paterson-Stockmeyer strategy
	{
	  // z^2
	  	Ctxt x2 = x;
	  	x2.square();
		// cout <<"the capacity of x^2 is " << x2.bitCapacity() << endl;
		DynamicCtxtPowers babyStep(x2, m_bs_num_comp);
		const Ctxt& x2k = babyStep.getPower(m_bs_num_comp);
		// cout <<"the capacity after baby steps is " << x2k.bitCapacity() << endl;


		DynamicCtxtPowers giantStep(x2k, m_gs_num_comp);
		// cout <<"the capacity after giant steps is " << x2k.bitCapacity() << endl;
		...
		if (m_gs_num_comp == (1L << NextPowerOfTwo(m_gs_num_comp))) 
		{
			...
			// cout << "capacity of ret in degPowerOfTwo is " << ret.bitCapacity() << endl;
	    }
	    else
	    {
			// cout << "I'm computing recursivePolyEval" << endl;
		  	recursivePolyEval(ret, m_univar_less_poly, m_bs_num_comp, babyStep, giantStep);
			// cout << "capacity of ret in recursivePolyEval is " << ret.bitCapacity() << endl;
		...
		ret.multiplyBy(x);
		// cout << "capacity after the last multiplyBy is " << ret.bitCapacity() << endl;
...
void Comparator::compare(Ctxt& ctxt_res, const Ctxt& ctxt_x, const Ctxt& ctxt_y) const
{
	...
	else // univariate circuit
	{
		...
		// if(m_verbose)
		// {
		// 	cout << "the decrypted x - y is: " << endl;
		// 	print_decrypted(ctxt_z);
		// 	cout << endl;
		// }
		...
		for (long iCoef = 0; iCoef < m_slotDeg; iCoef++)
		{
			...
			double log_q = ctxt_tmp.logOfPrimeSet();
    		double capacity = ctxt_tmp.capacity();
    		NTL::xdouble noise_bnd;
    		noise_bnd = ctxt_tmp.totalNoiseBound();
    		// std::cout << "after comparison:" << std::endl;
    		// std::cout << "log q = " << log_q << std::endl;
    		// std::cout << "noise bound = " << noise_bnd << std::endl;
...
//   unsigned long numbers_size = nslots / m_expansionLen;
  ...
  // number of slots occupied by encoded numbers
  unsigned long occupied_slots = numbers_size * m_expansionLen;
  //unsigned long occupied_slots = 2;

  //encoding base, ((p+1)/2)^d
  //if 2-variable comparison polynomial is used, it must be p^d
  unsigned long enc_base = (p + 1) >> 1;// (p + 1)/2
  if (m_type == BI || m_type == TAN)
  {
  	enc_base = p;
  }

  cout << "enc_base = " << enc_base << endl
    << "m_slotDeg (field extension degree) = " << m_slotDeg << endl;
  ...
  for (int run = 0; run < runs; run++)
  {
    ...
    for (int i = 0; i < numbers_size; i++)
    {
      input_x = 32; //distr_u(eng) % input_range;
      input_y = 44; //distr_u(eng) % input_range;
      ...
      if(m_verbose)
      {
      	cout << "m_expansionLen (maximal number of digits in a number) = " 
		  << m_expansionLen <<" \n Input decomposition into digits" << endl;
      	for(int j = 0; j < m_expansionLen; j++)
      	{
...
// compute the argmax matrix
void Comparator::argmax_matrix_later(vector<vector<Ctxt>>& ctxt_mat, const vector<Ctxt>& ctxt_in) const 
{
	long n = ctxt_in.size();
	ctxt_mat.resize(n);
	PtxtArray pt(ctxt_in[0].getContext().getEA());
	pt.load(1);
	
	for(long i = 0; i < n; i++){
		Ctxt ct_tmp(ctxt_in[0].getPubKey());
		for (long j = 0; j < n; j++){
			if (i > j){
				compare(ct_tmp, ctxt_in[i], ctxt_in[j]);
				ctxt_mat[i].push_back(ct_tmp);
			}
		}
...
void Comparator::argmax_later(Ctxt& ctxt_res, const vector<Ctxt>& ctxt_in) const
{
	long n = ctxt_in.size();
	vector<vector<Ctxt>> ctxt_matrix;
	argmax_matrix_later(ctxt_matrix, ctxt_in);
	PtxtArray pt(ctxt_in[0].getContext().getEA());
	pt.load(0);
	pt.encrypt(ctxt_res);
	for (long j = 0; j < n; j++){
...
void Comparator::argmax_matrix_former(vector<vector<Ctxt>>& ctxt_mat, const vector<Ctxt>& ctxt_in) const
{
	long n = ctxt_in.size();
	ctxt_mat.resize(n);
	PtxtArray pt(ctxt_in[0].getContext().getEA());
	...
	for(long i = 0; i < n; i++){
		...
		for (long j = 0; j < n; j++){
			if (i > j){
				compare(ct_tmp, ctxt_in[j], ctxt_in[i]);
				ctxt_mat[i].push_back(ct_tmp);
			}
		}
...
// argmax, if a = [1, 2, 3, 15, 15], it returns 3, i.e., the 4th is the maximum
void Comparator::argmax_former(Ctxt& ctxt_res, const vector<Ctxt>& ctxt_in) const{
	long n = ctxt_in.size();
	vector<vector<Ctxt>> ctxt_matrix;
	argmax_matrix_former(ctxt_matrix, ctxt_in);
	PtxtArray pt(ctxt_in[0].getContext().getEA());
	pt.load(0);
	pt.encrypt(ctxt_res);
	for (long i = 0; i < n; i++){
...
void Comparator::argmax(Ctxt& ctxt_res, const vector<Ctxt>& ctxt_in, const int former_later) const
{
	if (former_later == 0){
		argmax_former(ctxt_res, ctxt_in);
	}else{
		argmax_later(ctxt_res, ctxt_in);
	}
}